<?php
/**
 * ColourTech Industries - Sample Request Inquiries
 */
session_start();
define('ADMIN_ACCESS', true);
require_once 'includes/config.php';

$pageTitle   = 'Sample Request Inquiries';
$breadcrumbs = ['Sample Requests' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) { setFlashMessage('error','Invalid security token.'); redirect('inquiries-samples.php'); }
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'update_status') {
        ...
        redirect('inquiries-samples.php');
    }
    if ($action === 'delete') {
        $id = (int)($_POST['id']??0);
...
$where  = ["subject LIKE 'Sample Request:%'"]; $params=[];
...
$total   =(int)$pdo->query("SELECT COUNT(*) FROM contact_inquiries WHERE subject LIKE 'Sample Request:%'")->fetchColumn();
$newCount=(int)$pdo->query("SELECT COUNT(*) FROM contact_inquiries WHERE status='new' AND subject LIKE 'Sample Request:%'")->fetchColumn();
$read    =(int)$pdo->query("SELECT COUNT(*) FROM contact_inquiries WHERE status='read' AND subject LIKE 'Sample Request:%'")->fetchColumn();
$replied =(int)$pdo->query("SELECT COUNT(*) FROM contact_inquiries WHERE status='replied' AND subject LIKE 'Sample Request:%'")->fetchColumn();
...
foreach ($inquiries as &$inq) {
    ...
    $productName = $p['product'] ?: preg_replace('/^Sample Request:\s*/i', '', $inq['subject'] ?? '');
    $inq['_parsed']    = $p;
...
<div class="page-header">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h4><i class="fas fa-paint-brush me-2" style="color:#e6a817;"></i>Sample Request Inquiries</h4>
            <p class="mb-0">Customers requesting colour / product samples</p>
        </div>
    </div>
</div>
...
<div class="row mb-4">
    <div class="col-md-3 col-sm-6 mb-3"><div class="stats-card">
        <div class="stats-icon" style="background:linear-gradient(135deg,#e6a817,#c88a00)"><i class="fas fa-paint-brush"></i></div>
        <h3><?php echo $total; ?></h3><p>Total Requests</p>
    </div></div>
    <div class="col-md-3 col-sm-6 mb-3"><div class="stats-card">
...
<div class="card mb-3"><div class="card-body py-3">
    <form method="GET" action="inquiries-samples.php" class="row g-3 align-items-end">
        <div class="col-md-4">
        ...
        <div class="col-md-auto">
            ...
            <a href="inquiries-samples.php" class="btn btn-secondary ms-2"><i class="fas fa-times me-2"></i>Clear</a>
        </div>
    </form>
</div></div>
...
<div class="card">
    <div class="card-header"><h5 class="mb-0">
        <?php echo (!empty($statusFilter)||!empty($searchFilter))?'Filtered ('.count($inquiries).')':'All Sample Requests ('.count($inquiries).')'; ?>
    </h5></div>
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover data-table">
                <thead><tr>
                    ...
                    <th width="40">#</th><th>Sender</th><th>Product Hierarchy</th><th>Company / Country</th>
                    <th>Application</th><th>Date</th><th>Status</th><th width="120">Actions</th>
                </tr></thead>
                <tbody>
                    <?php if(empty($inquiries)): ?>
                        <tr><td colspan="9" class="text-center py-4 text-muted"><i class="fas fa-paint-brush fa-2x mb-2 d-block"></i>No sample requests found.</td></tr>
                    <?php endif; ?>
                    <?php foreach($inquiries as $i=>$inq): ?>
                        ...
                        $product = preg_replace('/^Sample Request:\s*/i','',$inq['subject']??'');
                        ?>
                        <tr class="<?php echo $inq['status']==='new'?'table-warning':''; ?>">
                                ...
                                <?php if($h['category_name']): ?>
                                    <div class="d-flex flex-column gap-1">
                                        ...
                                        <strong class="text-dark" style="font-size:0.9rem;">
                                            <i class="fas fa-box-open me-1 text-warning"></i>
                            ...
                            <td>
                                <?php if(!empty($p['company'])): ?><small><i class="fas fa-building me-1 text-muted"></i><?php echo htmlspecialchars($p['company']); ?></small><?php endif; ?>
                                <?php if(!empty($p['country'])): ?><br><small class="text-muted"><i class="fas fa-globe me-1"></i><?php echo htmlspecialchars($p['country']); ?></small><?php endif; ?>
                                <?php if(empty($p['company'])&&empty($p['country'])): ?><span class="text-muted">—</span><?php endif; ?>
                            </td>
                            <td><?php echo !empty($p['application'])?'<span class="badge bg-light text-dark border">'.htmlspecialchars($p['application']).'</span>':'<span class="text-muted">—</span>'; ?></td>
...
<div class="modal fade" id="viewModal" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title"><i class="fas fa-paint-brush me-2"></i>Sample Request Details</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="row mb-3">
                <div class="col-md-6"><label class="form-label text-muted small">Product</label><p class="mb-0 fw-bold" id="vProduct">—</p></div>
            ...
            <div class="row mb-3">
                <div class="col-md-4"><label class="form-label text-muted small">Company</label><p class="mb-0" id="vCompany">—</p></div>
                <div class="col-md-4"><label class="form-label text-muted small">Country</label><p class="mb-0" id="vCountry">—</p></div>
                <div class="col-md-4"><label class="form-label text-muted small">Used Application</label><p class="mb-0" id="vApplication">—</p></div>
            </div>
            <div class="mb-3" id="vMsgRow"><label class="form-label text-muted small">Message</label><div class="p-3 bg-light rounded" id="vMessage" style="white-space:pre-wrap;min-height:60px;">—</div></div>
...
function viewInquiry(d){currentId=d.id;const p=d._parsed||{};const h=d._hierarchy||{};
    ...
    document.getElementById('vProduct').textContent=(h.product_name||(d.subject||'').replace(/^Sample Request:\s*/i,''))||'—';
    ...
    document.getElementById('vApplication').textContent=p.application||'—';
    document.getElementById('vIP').textContent=d.ip_address||'—';
    const msg=p.message||'';
    document.getElementById('vMsgRow').style.display=msg.trim()?'block':'none';
    document.getElementById('vMessage').textContent=msg;
    document.getElementById('vReplyBtn').href=`mailto:${d.email}?subject=Re: ${encodeURIComponent(d.subject||'Sample Request')}`;
